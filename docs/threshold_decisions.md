...
### 7.22 Phase 1 — `rag_corpus` chunk staleness thresholds

Not scoring thresholds (nothing here feeds a mart), but the same "state the source, revisit when wrong" discipline applies to the document-corpus pipeline's `chunks_with_staleness` view (`ingestion/load_bigquery.py`).

| `source_tier` | Threshold | Derivation |
|---|---|---|
| `sec_filing` | 400 days | Annual filing cycle (~365 days) plus filing lag — Kroger and Walmart both file 10-Ks 6-8 weeks after fiscal year-end, so a prior-year 10-K is expected to still be the *latest available* filing for roughly 400 days after its own filing date, not 365. |
| `earnings_call` | 120 days | Quarterly reporting cycle (~90 days) plus a buffer for the next call's scheduling lag, so a transcript isn't flagged stale in the few weeks between quarter-end and the next call actually happening. |
| `industry_research` | 180 days | **Arbitrary — a convention pending evidence, not a derivation**, same status as `retail_healthy_pct_divisor = 7.5`. FMI/NielsenIQ reports have no fixed publication cadence this project has observed; 180 days is a reasonable-sounding half-year guess, not measured. Revisit once actual report cadence is observed across 2+ real reports. |
| `gov_methodology` | never stale | Methodology (how a series/index is defined) doesn't have a shelf life the way a data point does; FRED/USDA/BLS revise methodology rarely and post-hoc, not on a schedule this project should try to guess at. |
| `internal_doc` | never stale by age | Freshness here is tracked by `content_hash` diffing on re-ingest (auto-reingest, per the content-hash drift policy below), not wall-clock age — the project's own docs changing is expected and detected directly, not inferred from a date. |

**content_hash drift policy, by `source_class`:** `external` — fail the ingestion run; upstream content changing is a signal to inspect before re-ingesting, not to silently absorb. `internal` — auto-reingest; the project's own docs are expected to change and each change is a deliberate commit, not an anomaly.

### 7.23 BUG #8 — zero-width-space paragraph separators silently merged every 10-K section, forcing text through a lossy lowercasing fallback

**The defect.** SEC 10-K HTML renders paragraph breaks as a line containing only a zero-width space (`​`), not a genuinely blank line. Python's `str.strip()` does not treat `​` as whitespace, so `ingestion/fetch_sec.py`'s first-pass line filter (`[l.strip() for l in text.split("\n") if l.strip()]`) kept every one of these separator lines as if it were real content. The effect: all natural paragraph boundaries within each 10-K Item were erased, and `ingestion/chunk.py`'s paragraph splitter (which looks for a blank-line gap, `\n\s*\n`) found none — each entire Item section (Kroger's Item 7 MD&A alone: ~80,000 characters) came through as a single "paragraph." That forced the oversized-paragraph fallback path: encode the whole thing to token ids, window-slice at the 512-token limit, and `tokenizer.decode()` each window back to text. `bge-base-en-v1.5` uses an **uncased** WordPiece tokenizer, so decoding lowercased every character and mangled curly quotes/punctuation that don't survive the decode round-trip — silently degrading the stored text for every chunk in both 10-Ks.

**How it was caught.** Not from a test — from an HF Hub log line during the actual ingestion run: `Token indices sequence length is longer than the specified maximum sequence length for this model (788 > 512)`. This looked like routine tokenizer noise (and mostly is, whenever a long paragraph is intentionally encoded before being split) but chasing it down turned up the real defect: a direct check on the raw fetched Kroger text showed only 3 paragraph breaks in a 150,244-character document via the blank-line splitter, when the un-filtered raw line list showed 2,547 blank-*looking* lines that the filter had wrongly kept.

**The fix.** `fetch_sec.py` now strips zero-width Unicode characters (`​`, `‌`, `‍`, `﻿`) before checking whether a line is blank, and — critically — keeps genuinely-blank lines as empty strings in the returned line list rather than dropping them, so paragraph boundaries survive into the reconstructed section text. Result: paragraph count for the Kroger 10-K went from 3 to 919; every 10-K chunk now packs under the 512-token target without the lossy fallback firing at all; original casing and punctuation preserved (spot-checked: `"The Kroger Co. (the "Company" or "Kroger")"` renders correctly, not `"the kroger co . ( the [UNK] company [UNK]..."`).

**CORRECTION (2026-08-31) — this fix was verified for Kroger only; it was NOT actually complete for both filings as originally reported here.** The zero-width-space stripping itself was correct, but Walmart's filer produces a structurally different HTML paragraph layout (no separator element between paragraphs at all, not a zero-width-space one that needed stripping) that this fix did not cover. 90 of Walmart's 96 chunks were still going through the same lossy lowercasing fallback described above, undetected, until found during unrelated Phase 2 work. See **#7.25** for the real root cause, the fix, and re-verification. The second finding below (decode/re-encode drift, `_enforce_max_tokens`) is unaffected by this correction — that part was and remains accurate for both filings.

**A second, related enforcement gap found in the same pass:** even before this fix, 8 chunks across the two 10-Ks (from the overlap-fallback windowing path, used for genuinely oversized paragraphs) landed at 513–515 tokens — a few tokens over the hard limit — because `tokenizer.decode()` followed by re-`encode()` is not always token-count-stable (WordPiece merges can shift by a few tokens on the round trip). These were being silently truncated by the embedding model at encode time, which is exactly the failure mode the chunker was designed to prevent. Fixed by adding a hard post-processing invariant in `chunk.py` (`_enforce_max_tokens`) that re-checks and re-trims *every* chunk regardless of which code path produced it, rather than trusting construction-time arithmetic alone. Verified: 0 chunks over 512 tokens corpus-wide after both fixes.

**Caught the fix's own side effect, correctly.** Re-running ingestion after this fix changed the extracted `raw_text` for both 10-Ks (more whitespace/paragraph structure preserved than before), which changed their `content_hash`. The external-source drift assertion (§7.22, built the same session) fired exactly as designed and refused to silently overwrite `kroger_10k`/`walmart_10k` — correctly can't distinguish "my own extraction code changed" from "the SEC filing changed upstream," and shouldn't try to. Resolved by inspecting (confirmed it was the known fix, not real drift), then manually clearing and reloading those two documents.

### 7.24 Baseline retrieval failure — target for the Phase 4 ablation study

Recorded so ablation work (chunk size / top-k / reranking) has a real, concrete failure to improve against rather than optimizing in the abstract.

**Named failure case.** Query: *"what competitive risks does Kroger identify in its 10-K"*. Expected: the chunk containing Kroger's `COMPETITIVE ENVIRONMENT` risk-factor discussion (Item 1A). Actual: that chunk does **not** appear in the top-3 `VECTOR_SEARCH` results — confirmed the chunk exists in `rag_corpus.chunks` with the correct text (`LOWER(chunk_text) LIKE '%food retailing%'` matches it directly), so this is a ranking/embedding-similarity failure, not a missing-content or chunking failure. The model consistently ranks Kroger's opening "Business" framing (Item 1) and MD&A introduction (Item 7) above the specific Item 1A risk paragraph for this phrasing. 9 of the other 9 known-answer smoke-test queries landed in the top-3; this is the one recorded gap.

**Secondary observation — duplicate values compete in retrieval.** Query *"why is the CV multiplier 0.16"* returns `internal_readme`'s copy of the value at rank 1 (README documents key config values for onboarding) with `internal_threshold_decisions`'s own canonical table only at rank 3 — both chunks are accurate, but the file the value is actually *justified* in did not win. Any doc set where the same number is legitimately repeated across an internal reference doc and a decision-record doc should expect this: retrieval ranks by textual/semantic similarity to the query, not by which source is authoritative for the number. Worth keeping in mind if a later phase needs "cite the authoritative source," not just "cite *a* correct source."

(This is the finding `get_threshold_rationale` — one of the Phase 2 agent's 6 tools — was built to sidestep by construction: it filters retrieval to `document_id = 'internal_threshold_decisions'` specifically, rather than searching the general internal-doc set. See the Phase 2 build report.)

### 7.25 BUG #9 — the #7.23 fix was incomplete: Walmart's 10-K has no paragraph-separator element at all, not a zero-width-space one

**How this was found.** Not from a test — from a casing sanity check run during unrelated Phase 2 agent-tool work, while hand-verifying the `search_external_context` tool's fixture. A `REGEXP_CONTAINS(chunk_text, r'[A-Z]{3,}')` count across `kroger_10k` and `walmart_10k` chunks showed 52/90 (58%) of Kroger's chunks preserving real casing, matching #7.23's claim — but only 6/96 (6%) of Walmart's did. The other 90 were lowercased with mangled punctuation (`'item 1. business general walmart inc. ( " walmart, " ...'`), the exact symptom #7.23 reported fixed for both filings.

**Root cause.** #7.23's fix (`fetch_sec.py::_clean_lines`) strips zero-width Unicode characters and keeps genuinely-blank lines as empty strings, so a real zero-width-space-only separator line between two paragraphs survives cleaning as `""` — the old assembly step, `"\n".join(lines[start:end])`, then produces the double-newline gap (`\n\n`) that `chunk.py`'s paragraph splitter (`\n\s*\n`) looks for. That is exactly what Kroger's filing needed and got.

Walmart's filer inserts no separator element between paragraphs at all. Confirmed by inspecting the raw `BeautifulSoup(...).get_text("\n")` output directly: each paragraph is its own single line, with zero blank or zero-width-only lines anywhere between them — nothing for `_clean_lines`'s fix to strip, because there was never a separator line there to begin with. The old single-`"\n"` join therefore put exactly one newline between every pair of Walmart paragraphs, never a `\n\n` gap. `chunk.py`'s splitter found zero paragraph breaks in the entire filing, and each `[ITEM n]` section went through the same oversized-paragraph token-window-encode/decode fallback #7.23 diagnosed, with the same lossy uncased-tokenizer-decode consequence.

This is a genuine structural difference between the two companies' EDGAR filing HTML (different filing agents/rendering pipelines), not a second occurrence of the zero-width-space defect. The #7.23 fix was correct as far as it went — it just never covered a case it wasn't designed for.

**The fix.** `fetch_10k_sections` no longer joins the raw, blank-preserving line list with a single `"\n"`. It filters to non-blank lines and joins survivors with an explicit `"\n\n"`:
```python
paragraph_lines = [line for line in lines[start:end] if line]
section_text = "\n\n".join(paragraph_lines)
```
Correct for both filings regardless of which structure produced the raw lines: every surviving line is already one paragraph-worth of text (one `get_text()` block/text-node), so an explicit `"\n\n"` between every pair guarantees a real gap whether or not the source HTML also happened to contain its own separator markup. `chunk.py`'s greedy paragraph-packer (`_pack_paragraphs`, packs up to `TARGET_TOKENS=400`) absorbs the resulting finer paragraph granularity — confirmed below, not assumed.

**Verification.**
- Paragraph count, before this fix -> after: Kroger 919 -> 1,295 (finer-grained, not merged — every extra split is still a genuine `get_text()` line boundary, not a spurious one). Walmart: effectively 3 (per #7.23's original diagnosis of the un-fixed state) -> 1,262.
- Max single paragraph length: Kroger 2,831 chars, Walmart 2,893 chars — both handled by the existing oversized-paragraph fallback plus the `_enforce_max_tokens` hard cap; max `token_count` across the reloaded Walmart chunks is exactly 512, the ceiling, confirming the cap is doing its job and nothing is silently truncated further.
- Casing, direct spot-check on the reloaded `walmart_10k` chunks: `'Walmart Inc. ("Walmart," the "Company" or "we") is a people-led...'` — correct casing and punctuation, not the old `'walmart inc. ( " walmart , " ...'`. Corpus-wide: `chunk_text LIKE '%walmart inc%'` (lowercase) now matches 0 of 110 `walmart_10k` chunks, down from 90 lowercased chunks in the un-fixed state.
- `content_hash` drift assertion (external-source policy, §7.20-era design): fired exactly as intended on the first re-extraction attempt — `LoadError: walmart_10k: content_hash changed for an EXTERNAL source...`. Confirmed this was the expected consequence of this fix, not real upstream 10-K content changing, before manually clearing and reloading — same precedent #7.23 itself used for both filings after its own fix.
- `kroger_10k` was deliberately **not** re-ingested in this pass — its stored chunks still reflect the pre-#7.25 extraction (919-paragraph version), which already had correct casing, so there was no live defect to fix there. Its `content_hash` **will** change (919 -> ~1,295-paragraph extraction) the next time it's re-ingested for any other reason; the drift assertion will fire then too and should be cleared the same way, with the same confirm-before-clearing discipline.
