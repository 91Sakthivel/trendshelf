...
**Scope note:** the stable-product-identifier fix (persisting SerpAPI's `us_item_id`/`product_id` instead of only `product_name`) is tracked separately, landing in its own commit this same session — see the entry immediately following this one once written.
